...
function MessageBubble({ role, content, images = [] }) {
  ...
  return (
    <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div
        className={`w-fit max-w-[92vw] md:max-w-[72%] px-4 pt-2.5 rounded-2xl break-word overflow-hidden leading-relaxed ${isUser ? "bg-linear-to-br from-indigo-500 to-violet-700 text-white rounded-tr-sm" : "text-slate-200 rounded-tl-sm"}`}
      >
        {images.length > 0 && (
      ...
      {lightBox && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-6">
          ...
          <img
            src={lightBox}
            className="max-w-[90vw] max-h-[85vh] rounded-2xl border-white/10 shadow-2xl object-contain"
          />
        </div>
      )}
    </div>
